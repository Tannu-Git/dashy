...
export const ContactsSection = () => {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 sm:p-5 border-b border-border sticky top-0 z-10 bg-background backdrop-blur-xl">
        <h1 className="text-[13px] font-semibold leading-5">Contacts</h1>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 sm:gap-[15px] w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-initial">
            <SearchIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#c4c4c4] w-3.5 h-3.5" />
            <Input
              type="text"
              placeholder="Search by name or email..."
              className="pl-10 h-[34px] text-[13px] border-input focus:border-border w-full sm:min-w-[328px] transition-all duration-200 focus:ring-2 focus:ring-primary/20"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
          </div>
          <Button className="h-[34px] px-2.5 py-[5px] bg-[#3c3c3c] hover:bg-[#3c3c3c]/90 rounded-[10px] text-[13px] font-semibold transition-all duration-200 hover:scale-105 hover:shadow-md group shrink-0">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 sm:p-5 border-b border-border sticky top-[73px] z-10 bg-background/95 backdrop-blur-xl">
        <div className="flex items-center gap-2.5 overflow-x-auto momentum-scroll pb-2 sm:pb-0 w-full sm:w-auto">
      ...
      <div className="flex-1 p-4 sm:p-10 overflow-auto touch-scroll">
            ...
            <div key={tag.id} className="flex flex-col gap-[15px]">
              ...
              <div className="bg-transparent rounded-[25px] space-y-4">
                {contacts.filter(contact => contact.tags.includes(tag.name)).map((contact) => (
                  <div
                    key={contact.id}
                    className="p-[15px] bg-card border border-border rounded-[15px] transition-all duration-200 hover:scale-[1.02] hover:shadow-lg cursor-pointer group touch-scroll"
                  >
